...
function readHighlightCoverage(value: unknown): { source: number; covered: number } | null {
  if (!isRecord(value)) return null;
  const source = typeof value.sourceHighlightCount === "number" ? value.sourceHighlightCount : null;
  const covered = typeof value.coveredHighlightCount === "number" ? value.coveredHighlightCount : null;
  return source !== null && covered !== null ? { source, covered } : null;
}
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[18rem_minmax(0,1fr)]">
        ...
        <section className="min-w-0 rounded-2xl border border-outline-variant/20 bg-surface-container-low p-4">
          {detailLoading ? <div className="flex min-h-64 items-center justify-center gap-2 text-sm text-on-surface-variant"><Loader2 className="h-5 w-5 animate-spin text-primary" />读取题目、答案与解析…</div> : !quiz ? <div className="flex min-h-64 items-center justify-center text-sm text-on-surface-variant">选择一份快测开始审核。</div> : <>
            ...
            <div className="mt-4 rounded-xl border border-outline-variant/20 bg-surface p-3 text-xs text-on-surface-variant"><p>{quiz.note_id ? "已绑定博客文章，发布后阅读助手可调用。" : "尚未绑定博客文章；批准后，发布讲义时会自动绑定。"}</p>{(() => { const coverage = readHighlightCoverage(quiz.self_check); return coverage && coverage.source > 0 ? <p className={`mt-1 ${coverage.covered > 0 ? "text-emerald-700" : "text-amber-700"}`}>正文高亮词覆盖：{coverage.covered}/{coverage.source}，请重点检查未覆盖的核心词。</p> : null; })()}</div>
