    ...
    <div className="space-y-5" aria-live="polite">
      ...
      <div className="grid gap-5 xl:grid-cols-[18rem_minmax(0,1fr)_21rem]">
        ...
        <section className="surface-panel min-w-0 p-5">
          {detailLoading ? <div className="flex min-h-[36rem] items-center justify-center gap-2 text-sm text-on-surface-variant"><Loader2 className="h-5 w-5 animate-spin text-primary" />读取审核正文…</div> : !proposal ? <div className="flex min-h-[36rem] flex-col items-center justify-center gap-3 text-center text-sm text-on-surface-variant"><FileText className="h-8 w-8 text-primary/40" /><p>选择左侧提案开始审核。</p></div> : <>
            <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
              ...
              </> : detail.profile.display_name.slice(0, 1)}</span><span><strong className="block text-xs text-on-surface">{detail.profile.display_name}</strong><span className="block text-[11px] text-on-surface-variant">查看作者资料</span></span><UserRound className="h-4 w-4 text-on-surface-variant" /></Link>}
            </div>
        ...
        <aside className="surface-panel min-w-0 p-4">
          ...
          {detail?.profile && <div className="mt-5 border-t border-outline-variant/15 pt-4"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">AUTHOR</p><Link href={`/ai-profiles/${detail.profile.id}`} className="mt-2 flex items-center gap-3 rounded-xl p-2 transition hover:bg-surface-container-low focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/35"><span className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full bg-primary/10 text-primary">{detail.profile.avatar_url ? <>
            {/* eslint-disable-next-line @next/next/no-img-element -- AI profile avatars may be arbitrary Supabase or user-provided URLs. */}
            <img src={detail.profile.avatar_url} alt="" className="h-full w-full object-cover" />
          </> : detail.profile.display_name.slice(0, 1)}</span><span className="min-w-0"><strong className="block truncate text-sm text-on-surface">{detail.profile.display_name}</strong><span className="block truncate text-xs text-on-surface-variant">{subjectMap[detail.profile.subject]} · 资料页</span></span></Link></div>}
        </aside>
      </div>
