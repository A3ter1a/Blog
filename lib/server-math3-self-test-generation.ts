...
import { normalizeMarkdownForWrite } from "./content-contract";
import {
...
export type Math3SelfTestGenerationStage = {
  phase: string;
  statusText: string;
...
type GenerateMath3SelfTestInput = {
  apiKey: string;
  ...
  signal?: AbortSignal;
  onStage?: (stage: Math3SelfTestGenerationStage) => Promise<void> | void;
};

export type Math3SelfTestGenerationResult = {
  paper: Math3SelfTestPaper;
  verification: Math3SelfTestVerification;
  tokensUsed: number;
};

export class Math3SelfTestPaperRejectedError extends Error {
  constructor(message: string, readonly tokensUsed = 0) {
    super(message);
    this.name = "Math3SelfTestPaperRejectedError";
  }
}

function repairPaperMarkdown(paper: Math3SelfTestPaper): Math3SelfTestPaper {
  return {
    ...paper,
...
async function auditQuestionGroup(
  ...
  options: { allowCorrections: boolean; signal?: AbortSignal },
  ...
  try {
    ...
    throw new Math3SelfTestPaperRejectedError(`${areaLabel}独立审校返回格式无效，试卷未保存`, tokensUsed);
  }
  ...
  try {
    ...
    throw new Math3SelfTestPaperRejectedError(`${areaLabel}：${message}`, tokensUsed);
  }
}

async function emitStage(
  onStage: GenerateMath3SelfTestInput["onStage"],
  stage: Math3SelfTestGenerationStage,
): Promise<void> {
  await onStage?.(stage);
}

export async function generateVerifiedMath3SelfTestPaper(
  input: GenerateMath3SelfTestInput,
): Promise<Math3SelfTestGenerationResult> {
  let totalTokensUsed = 0;
  const { apiKey, generationModel, reviewerModel, mode, difficulty, signal, onStage } = input;
  const config = getMath3SelfTestConfig(mode, difficulty);
  const realPaperProfile = getMath3RealPaperProfile(mode);

  await emitStage(onStage, {
    phase: "正在生成命题草稿",
    statusText: "DeepSeek 正在按固定题号蓝图生成原创试卷",
    progressCurrent: 0,
    progressTotal: 3,
  });
...
请严格按计划生成 ${config.questionPlan.length} 道题。生成内容随后会被另一组审校调用逐题独立验算，不确定的题会被拒绝。`;
  ...
  try {
    ...
    await emitStage(onStage, {
      phase: "正在修复命题格式",
      statusText: "命题内容已返回，正在修复 JSON 格式后进入独立审校",
      progressCurrent: 1,
      progressTotal: 3,
    });
    const repaired = await callDeepSeek(
      ...
      { temperature: 0, maxTokens: mode === "full" ? 16000 : 8000, responseFormat: "json_object", signal },
    );
  ...
  if (draftPaper.questions.length !== config.questionPlan.length) {
    throw new Math3SelfTestPaperRejectedError(`命题题量不完整：需要 ${config.questionPlan.length} 题，实际 ${draftPaper.questions.length} 题`, totalTokensUsed);
  }

  await emitStage(onStage, {
    phase: "正在进行分科独立审校",
    statusText: "微积分、线性代数与概率统计正在并行独立验算",
    progressCurrent: 1,
    progressTotal: 3,
  });
  ...
  const auditResults = settledAuditResults.flatMap((result) => {
    ...
    if (result.reason instanceof Math3SelfTestPaperRejectedError) totalTokensUsed += result.reason.tokensUsed;
    return [];
  });
  ...
  if (failedAudit?.status === "rejected") {
    ...
    throw new Math3SelfTestPaperRejectedError(message, totalTokensUsed);
  }
  ...
  if (correctedByIndex.size !== corrections.length) {
    throw new Math3SelfTestPaperRejectedError("独立审校返回了重复修正题号，试卷未保存", totalTokensUsed);
  }
  ...
  if (structuralIssues.length > 0) {
    throw new Math3SelfTestPaperRejectedError(`最终结构校验未通过：${structuralIssues.slice(0, 5).join("；")}`, totalTokensUsed);
  }

  await emitStage(onStage, {
    phase: "正在进行高风险二次终审",
    statusText: "解答题、难题与修正题正在执行不可再修正的最终验算",
    progressCurrent: 2,
    progressTotal: 3,
  });
  ...
  try {
    const finalGateResult = await auditQuestionGroup(
      ...
      "高风险题二次终审",
      finalGateQuestions,
      { allowCorrections: false, signal },
    );
    totalTokensUsed += finalGateResult.tokensUsed;
  } catch (error) {
    if (error instanceof Math3SelfTestPaperRejectedError) {
      totalTokensUsed += error.tokensUsed;
      throw new Math3SelfTestPaperRejectedError(error.message, totalTokensUsed);
    }
    throw error;
  }
  ...
  await emitStage(onStage, {
    phase: "双重审校已通过",
    statusText: "命题、分科审校和高风险终审均已完成，正在保存结果",
    progressCurrent: 3,
    progressTotal: 3,
  });
  return { paper, verification, tokensUsed: totalTokensUsed };
}
