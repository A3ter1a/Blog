...
assert.match(createRoute, /getAdminRequestContext\(req\)/, "Markdown 创建路由必须经过管理员鉴权");
...
assert.match(createRoute, /internalJobLeaseSchemaAvailable/, "Markdown 审阅必须先检查 lease RPC 实际可用性");
assert.match(envExample, /WP3_INTERNAL_JOB_LEASE_ENABLED=false/, "旧版 rollout 兼容项必须保持安全默认值");
...
assert.match(advanceRoute, /export const maxDuration = 900/, "单分块路由必须声明受控最大时长");
...
assert.doesNotMatch(createPage, /\/api\/ai\/document-markdown-review/, "schema pending 时不得回退到不可恢复的同步任务");
assert.match(createRoute, /不能安全开始 Markdown 审阅/, "持久能力不可用时必须 fail closed 并返回明确错误");
...
console.log("WP3 internal Markdown review worker assets verified: auth, lease, fail-closed, redaction, checksum, confirmation");
