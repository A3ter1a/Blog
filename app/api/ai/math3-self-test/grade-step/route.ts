...
import { createMath3StepGradeJob, internalJobLeaseSchemaAvailable } from "@/lib/server-internal-job-runner";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const question = asRecord(body.question) as Math3StepGradeQuestionSnapshot;
    const step = asRecord(body.step) as Math3StepGradeRubricSnapshot;
    ...
    const ledger = await createMath3StepGradeJob(auth.context.supabase, {
    ...
    if (ledger.availability !== "synced" || !ledger.data) {
      return NextResponse.json({ error: "数学三分步评分任务登记失败", availability: ledger.availability, success: false }, { status: 503 });
    }
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "分步评分任务创建失败", success: false }, { status: 500 });
  }
}
