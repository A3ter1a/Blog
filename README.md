...
生产环境不要继续使用旧的 `supabase-init.sql`。当前数据库与 RLS 的标准入口在 `supabase/migrations/`：
...
npm run verify:rls-assets          # 检查本地 Supabase 迁移/RLS 文件
npm run verify:production-security # 检查公网关键安全门
```
